			...
			<div class="span12">
				<div class="content">
					...
					if (isset($_POST['input'])) {
						...
						$check_duplicate = mysqli_query($conn, "SELECT * FROM persona WHERE Nombre = '$nombre' AND Titulo = '$titulo'");
						if (mysqli_num_rows($check_duplicate) > 0) {
							echo '<div class="alert alert-warning alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>La persona ya existe en el sistema.</div>';
						} else {
							$insert = mysqli_query($conn, "INSERT INTO persona(Id, Nombre, Titulo) VALUES(NULL,'$nombre','$titulo')");
							if ($insert) {
								...
								header("Location: alta_persona.php");
							} else {
					...
					<blockquote>
						<h3>BCFEXA - Agregar Personas </h3>
					</blockquote>
					<form name="form1" id="form1" class="form-horizontal row-fluid" action="nueva_persona.php"
						method="POST">

						<div class="control-group">
							<label class="control-label" for="extracto">Persona que participa de la Actividad</label>
							<div class="controls">
								<input name="nombre" id="nombre" class=" form-control span8 tip" type="text"
									placeholder="Ingrese APELLIDO(s), Nombre(s) . Ej.: Olguin, Luis Alberto" required />
							</div>
						</div>
						<div class="control-group">
							<label class="control-label" for="titulo">Título</label>
							<div class="controls">
								<input name="titulo" id="titulo" class=" form-control span8 tip" type="text"
									placeholder="Indique el título que figura en la resolución" />
							</div>
						</div>

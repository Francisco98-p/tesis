...
<body>
			...
			<div class="span12">
				<div class="content">
					...
					if (isset($_POST['input'])) {
						...
						$check_query = "SELECT COUNT(*) as count FROM unidadejecutora WHERE Unidad = '$unidad'";
						$check_result = mysqli_query($conn, $check_query);
						$check_row = mysqli_fetch_assoc($check_result);

						if ($check_row['count'] > 0) {
							echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Ya existe una unidad ejecutora con ese nombre.</div>';
						} else {
							$insert = mysqli_query($conn, "INSERT INTO unidadejecutora(Id, Unidad) VALUES(NULL,'$unidad')");
							if ($insert) {
					...
					<blockquote>
						<h3>BCFEXA - Agregar Unidad Ejecutora </h3>
					</blockquote>
					<form name="form1" id="form1" class="form-horizontal row-fluid" action="nueva_unidadejecutora.php"
						method="POST">

						<div class="control-group">
							<div class="controls">
								<input name="unidad" id="unidad" class=" form-control span8 tip" type="text"
									placeholder="Unidad de la FCEFN" required />
							</div>
						</div>
	...
	<div class="footer span-12">
		<div class="container">
			<center> <b class="copyright"> BCFEXA - IdeI &copy;
					<?php echo date("Y") ?>
				</b></center>
		</div>
	</div>
